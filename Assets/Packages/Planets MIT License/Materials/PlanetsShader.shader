Shader "Custom/PlanetsShader"
{
	...
	SubShader
	{
		...
		inline half4 LightingCustomLighting(SurfaceOutputStandardSpecularCustom s, fixed3 lightDir, fixed3 viewDir, fixed atten)
		{
			...
			fixed diffuseMultiplier = max(0, dot(s.Normal, lightDir)) * atten;
			fixed nh = max(0, dot(s.Normal, h));
			fixed spec = pow(nh, 4) * s.Smoothness;
			fixed4 c;
			c.rgb = ((lerp(s.NightColor, s.Albedo * _LightColor0.rgb * diffuseMultiplier, saturate(_NightTransitionVariable * diffuseMultiplier))) +
				(_LightColor0.rgb * s.Specular.rgb * _SpecularPower * spec));
			c.a = s.Alpha;
